...
const MyCard= ({data , setPage}: any) => {
  ...
  const goToLink = (input : string) : any=> {
      switch (input) {
        case "courses":
        setPage("courses")
      }
  ...
  return (
    ...
    <Image src={vector1} className='h-7 w-6 absolute top-[2rem]' />
        <Box className=' flex flex-col gap-[1.5rem]'>
            ...
            <Box className='hover:cursor-pointer hover:-translate-y-1 hover:scale-110  duration-300 ' onClick={()=>goToLink((data.name).toLowerCase())}>
